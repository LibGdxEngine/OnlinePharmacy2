...
public class CanvasView extends View {
    ...
    public CanvasView(Context context, Order order, List<Product> productList, User user ) {
        super(context);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < productList.size() ; i++) {
            ...
            canvas.drawText(" " +Integer.toString(productList.get(i).getPrice()), (float) (getWidth() * 0.4), (float) (getHeight() * 0.05 * (i + DISTANCE_BETWEEN_TEXTS)), mTextPaint);
            canvas.drawText(" " +countList.get(i), (float) (getWidth() * 0.6), (float) (getHeight() * 0.05 * (i + DISTANCE_BETWEEN_TEXTS)), mTextPaint);
            int totalPrice = productList.get(i).getPrice() * Integer.parseInt(countList.get(i));
            canvas.drawText(" " + totalPrice, (float) (getWidth() * 0.8), (float) (getHeight() * 0.05 * (i + DISTANCE_BETWEEN_TEXTS)), mTextPaint);
        }
        mTextPaint.setColor(Color.BLUE);
        canvas.drawText("Total : " + order.getTotalPrice() , (float) (getWidth() * 0.7), (float) (getHeight() * 0.05 * (productList.size() + DISTANCE_BETWEEN_TEXTS)), mTextPaint);
        canvas.drawText(TimeStampFormatter.timeStampToString((long)order.getTimeStamp()) , (float) (getWidth() * 0.0), (float) (getHeight() * 0.05 * (productList.size() + DISTANCE_BETWEEN_TEXTS * 2)), mTextPaint);
    }
