package com.devahmed.techx.foxmart.Screens.AddProducts.AddProductUseCase;
...
//        productSubCategory.setId(key);
    ...
    public void updateProduct(final Product updatedProduct, Uri pickedImage){
        ...
        try{
            if(!(((Integer)updatedProduct.getPrice()) instanceof Integer)){
                notifyInputError("Price is not valid");
                return;
            }
        }catch (Exception e){
    ...
    public void updateProductWithOldImage(final Product updatedProduct){
        if(!isValid(updatedProduct.getTitle())){
            notifyInputError("Product name is not valid");
            return;
        }
