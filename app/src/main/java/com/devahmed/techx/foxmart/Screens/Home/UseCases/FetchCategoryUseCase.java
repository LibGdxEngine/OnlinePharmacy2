package com.devahmed.techx.foxmart.Screens.Home.UseCases;
...
public class FetchCategoryUseCase extends BaseObservableMvcView<FetchCategoryUseCase.Listener> {
    ...
    public void getOffers(Context context){
        ...
        final String userBranch = sharedPreferences.getString("userBranch" , "-M277dfeyOhPe37PGU4L");
        //get products related to this branch only
        ...
        offersDatabaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                brancProducts = new ArrayList<>();
                for (DataSnapshot postSnap: dataSnapshot.getChildren()) {
                    Product model = postSnap.getValue(Product.class);
                    if(model.getBranch().equals(userBranch)){
                        brancProducts.add(model);
                    }
                }
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        ...
                        for (DataSnapshot postSnap: dataSnapshot.getChildren()) {
                            ...
                            if(model.getType() == ProductSubCategory.TYPE_PRODUCT){
                                for (int i = 0; i <brancProducts.size() ; i++) {
                                    if(brancProducts.get(i).getId().equals(model.getId())
                                            && brancProducts.get(i).getBranch().equals(userBranch)){
